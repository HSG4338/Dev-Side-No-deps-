...
def main():
    ...
    if len(args) == 3:
        try:
            print(calculate(args[0], float(args[1]), float(args[2])))
        except ValueError as e:
            print(f"Error: {e}", file=sys.stderr)
            sys.exit(1)
    else:
        print("Calculator REPL — type 'quit' to exit. Usage: op a b")
        while True:
            try:
                line = input("> ").strip()
            ...
            parts = line.split()
            if len(parts) != 3:
                print("Usage: op a b  (e.g. add 3 5)")
                continue
            try:
                print(calculate(parts[0], float(parts[1]), float(parts[2])))
            except ValueError as e:
                print(f"Error: {e}")
